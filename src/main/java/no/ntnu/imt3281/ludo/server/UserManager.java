 ...
 * Note: Class is threadsafe, as access to the database can happen concurrently.
...
public class UserManager {
    ...
    public void createUser(JSONArray requests, JSONArray successes, JSONArray errors, Queue<Message> events) {
        applyFirstOrderFilter(RequestType.CREATE_USER_REQUEST, requests, errors);
    ...
    public void deleteUser(JSONArray requests, JSONArray successes, JSONArray errors, Queue<Message> events) {
        applyFirstOrderFilter(RequestType.DELETE_USER_REQUEST, requests, errors);
        MessageUtility.each(requests, (requestID, request) -> {
    ...
    public void updateUser(JSONArray requests, JSONArray successes, JSONArray errors, Queue<Message> events) {
        applyFirstOrderFilter(RequestType.UPDATE_USER_REQUEST, requests, errors);
        MessageUtility.each(requests, (requestID, request) -> {
     ...
     * @param successes JSONArray where the successes should be appended to.
     * @param errors    JSONArray where the errors should be appended to.
     * @param events    Queue where any events generated by this function should be put.
     */
    public void getUser(JSONArray requests, JSONArray successes, JSONArray errors, Queue<Message> events) {
        applyFirstOrderFilter(RequestType.GET_USER_REQUEST, requests, errors);
        MessageUtility.each(requests, (requestID, request) -> {
            var userID = request.getInt(FieldNames.USER_ID);
            try {
    ...
    public void getUserRange(JSONArray requests, JSONArray successes, JSONArray errors, Queue<Message> events) {
        applyFirstOrderFilter(RequestType.GET_USER_RANGE_REQUEST, requests, errors);
        MessageUtility.each(requests, (requestID, request) -> {
    ...
    public void logInUser(JSONArray requests, JSONArray successes, JSONArray errors, Queue<Message> events) {
        if (requests.length() < 1) {
            Logger.log(Logger.Level.WARN, "Someone tried to log in without sending in any requests");
            return;
        }
    ...
    public void logOutUser(JSONArray requests, JSONArray successes, JSONArray errors, Queue<Message> events) {
        applyFirstOrderFilter(RequestType.LOGOUT_REQUEST, requests, errors);
        MessageUtility.each(requests, (requestID, request) -> {
            var userID = request.getInt(FieldNames.USER_ID);
            try {
    ...
    public void friend(JSONArray requests, JSONArray successes, JSONArray errors, Queue<Message> events) {
        applyFirstOrderFilter(RequestType.FRIEND_REQUEST, requests, errors);
        MessageUtility.each(requests, (requestID, request) -> {
    ...
    public void unfriend(JSONArray requests, JSONArray successes, JSONArray errors, Queue<Message> events) {
        applyFirstOrderFilter(RequestType.UNFRIEND_REQUEST, requests, errors);
        MessageUtility.each(requests, (requestID, request) -> {
            var userID = request.getInt(FieldNames.USER_ID);
            var friendID = request.getInt(FieldNames.OTHER_ID);
    ...
    public void ignore(JSONArray requests, JSONArray successes, JSONArray errors, Queue<Message> events) {
        applyFirstOrderFilter(RequestType.IGNORE_REQUEST, requests, errors);
        MessageUtility.each(requests, (requestID, request) -> {
            var userID = request.getInt(FieldNames.USER_ID);
            var friendID = request.getInt(FieldNames.OTHER_ID);
    ...
    public void getFriendRange(JSONArray requests, JSONArray successes, JSONArray errors, Queue<Message> events) {
        applyFirstOrderFilter(RequestType.GET_FRIEND_RANGE_REQUEST, requests, errors);
        MessageUtility.each(requests, (requestID, request) -> {
            var userID = request.getInt(FieldNames.USER_ID);
            var pageIdx = request.getInt(FieldNames.PAGE_INDEX);
            try {
    ...
    /**
     * Applies the first order filter in the requests within requests, removes the erroneous ones and
    ...
    /**
     * Checks whether or not a token exists within the database, i.e. is tied to a user.
     * @param token The token to check
     * @return True if the token exists in the database, false otherwise.
     */
    ...
    /**
     * Checks whether or not two users are friends.
     * @param userID The user to check if is friend with otherID
     * @param otherID the id of the user to check if is friend with userID
     * @return
     */
    ...
    /**
     * Checks if username is between 4 and 50 symbols, and only contains letters
    ...
    /**
     * Generates a 32 byte salt to be used for hashing passwords
    ...
    /**
     * Generates a random token that can be mapped to users.
     * The values are a subset of ASCII, with the focus on not containing stuff like endlines, which will mess up the json objects.
     * @return
     */
     ...
     * @param password The password (in plaintext) to hash.
